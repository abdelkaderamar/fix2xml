#include <gtest/gtest.h>

#include "converter/fixml2fix_converter.hxx"
#include "util/fix_env.hxx"
#include "tools/test_util.hxx"

#include <boost/log/trivial.hpp>

#include <quickfix/fix50sp2/ApplicationMessageReport.h>

#include <list>
#include <set>
#include <string>
#include <utility>

using namespace std;
using namespace fix2xml;

TEST ( ApplicationMessageReport_message_t, set_fields)
{

  fixml2fix_converter converter {"../spec/fix/FIX50SP2.xml", "../spec/xsd/fixml-main-5-0-SP2.xsd"};
  auto& fixml_dict = converter.fixml_dico();
  ASSERT_TRUE(converter.init());

  xml_element elt {"ApplMsgRpt" };

  list<multiset<string>> all_values;
  multiset<string> all_compo_names;
  multiset<string> ApplicationMessageReport_message_t_0;
  elt.add_attribute("ApplRptID", "ApplReportID_t_1251118865");
  elt.add_attribute("ApplReqID", "ApplReqID_t_360101534");
  elt.add_attribute("ApplRptTyp", "0");
  elt.add_attribute("Txt", "Text_t_1659715646");
  elt.add_attribute("EncTxtLen", "990177250");
  elt.add_attribute("EncTxt", "EncodedText_t_2063332741");
  all_values.push_back(ApplicationMessageReport_message_t_0);
  all_compo_names.insert("ApplicationMessageReport_message_t");

  cout << "////////////////////////////////////////////" << endl;
  cout << elt.to_string() << endl;
  cout << "////////////////////////////////////////////" << endl << endl;

}
